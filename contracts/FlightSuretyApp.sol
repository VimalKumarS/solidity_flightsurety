...
contract FlightSuretyApp {
  ...
  */
  ...
  function fetchFlightStatus(address airline, string calldata flight, uint256 timestamp) external {
    ...
    // ResponseInfo storage response = ResponseInfo({requester: msg.sender, isOpen: true});
  ...
  function pay() public requireIsOperational {
    ...
    mapping(string => OracleStatus[]) responses; 
    ...
    function submitOracleResponse(
        ...
        // responses[key].push(OracleStatus({statusCode:statusCode, addr:(msg.sender)}));
        // oracleResponses[key].responses[statusCode].push(msg.sender);
        string memory _key= string(abi.encodePacked(key,' ',statusCode));
            responses[_key].push(OracleStatus({addr:(msg.sender)}));
        // Information isn't considered verified until at least MIN_RESPONSES
        // oracles respond with the *** same *** information
        emit OracleReport(airline, flight, timestamp, statusCode);
        if (
            // oracleResponses[key].responses[statusCode].length >= MIN_RESPONSES
             responses[_key].length >= MIN_RESPONSES
        ) {
